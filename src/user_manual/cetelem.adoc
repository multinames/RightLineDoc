...
*    модуль веб-интерфейса dashboard-app - rpm пакет `sbp-connect-cetelem-dashboard-{версия_релиза}~RELEASE.x86_64.rpm`
*    папку с файлами `liquibase` для настройки базы данных (БД)
*    папку `documentation` с файлами документации
...
`sudo service service sbp-connect-cetelem-dashboard start` - запуск службы dashboard-app

`sudo service sbp-connect-cetelem-dashboard stop` - остановка службы dashboard-app

`sudo service sbp-connect-cetelem-dashboard status` - проверка состояния client-app
